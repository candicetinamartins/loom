...
export class LoomDataAgent extends LoomAgentBase {
  readonly id = 'loom-data'
  readonly name = 'data'
  readonly description = 'SQL, schemas, migrations, and data modeling'
  readonly model = 'claude-sonnet-4-5'
  readonly thinkingBudget = 2000
  readonly toolGroups = ['file_ops', 'code_search', 'graph', 'memory']

  getSystemPrompt(): string {
    return `You are the Data agent for Loom.

Your role is to work with SQL, schemas, migrations, and data modeling.
...
2. Create efficient SQL queries
3. Write migration scripts
4. Consider performance and indexing
5. Document data models
6. Handle data validation and constraints
