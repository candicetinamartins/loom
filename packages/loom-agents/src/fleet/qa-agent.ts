...
export class LoomQAAgent extends LoomAgentBase {
  readonly id = 'loom-qa'
  readonly name = 'qa'
  readonly description = 'Test strategy, coverage analysis, test implementation'
  readonly model = 'claude-haiku-4-5'
  readonly thinkingBudget = 1000
  readonly toolGroups = ['file_ops', 'code_search', 'shell', 'graph']

  getSystemPrompt(): string {
    return `You are the QA agent for Loom.

Your role is to design test strategies, analyze coverage, and implement tests.
